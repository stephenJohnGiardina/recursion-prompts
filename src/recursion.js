...
var reverse = function(string) {
	...
	let reversed = string[string.length - 1];
...
var fizzBuzz = function(n) {
	...
	return fizzBuzz(n - 1).concat(list);
};
...
var countOccurrence = function(array, value) {
	...
	if (newArray[newArray.length - 1] === value) {
		sum++;
	}
	newArray.splice(newArray.length - 1, 1);
	return sum + countOccurrence(newArray, value);
};
...
var rMap = function(array, callback) {
	...
	map.push(callback(newArray[0]));
	newArray.splice(0, 1);
	return map.concat(rMap(newArray, callback));
};
...
var capitalizeWords = function(array) {
	...
	list.push(array[0].toUpperCase());
	array.splice(0, 1);
	return list.concat(capitalizeWords(array));
};
...
var capitalizeFirst = function(array) {
	...
	list.push(array[0][0].toUpperCase() + array[0].slice(1));
	array.splice(0, 1);
	return list.concat(capitalizeFirst(array));
};
...
var flatten = function(array) {
	...
	for (let i = 0; i < array.length; i++) {
		if (Array.isArray(array[i])) {
			list = list.concat(flatten(array[i]));
		} else {
			list.push(array[i]);
		}
	}
...
var binarySearch = function(array, target, min, max) {
	...
	if (target > array[avg]) {
		...
		return binarySearch(array, target, min, avg);
	}
	...
	if (Array.isArray(input)) {
		...
		for (let i = 0; i < keys.length; i++) {
			if (typeof input[keys[i]] === 'object') {
				result[keys[i]] = clone(input[keys[i]]);
			} else {
